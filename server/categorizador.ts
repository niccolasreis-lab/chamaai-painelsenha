 ...
 */
export function mapOldCategoryToNew(oldCat: string, descricao?: string): string {
...
export function migrateDatabaseAndConfigs(db: any) {
  ...
  try {
    const migrado = db.prepare("SELECT valor FROM configuracoes WHERE chave = 'categoria_migracao_v2'").get() as any;
  ...
  const updateJSONFile = (filePath: string) => {
    try {
